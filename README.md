...
### `qca-nss-drv` reload panics in sysctl registration

Reloading the driver (rmmod then modprobe `qca_nss_drv`) panics in
`nss_stats_register_sysctl` → `strlen(NULL)`: a NULL `procname` / missing
terminator in a sysctl table (same class as the ECM `net/ecm` sysctl fix). Only
fires on a reload — the module normally loads once per boot. Fix: terminate the
sysctl table.
...
to reset the offload plane in place — only a full reboot does. Compounds the
reload-path issue above.
